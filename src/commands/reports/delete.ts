/**
 * `freelo reports delete <id>... [--ids <list>] [--stdin] [--yes] [--dry-run]`
 * (R22, spec 0034).
 *
 * Deletes one or more work reports via `DELETE /work-reports/{id}` (yaml :3144-3171).
 * Destructive — gates on the shared `confirmDestructive` helper (R13).
 *
 * Idempotency (spec 0034 decision 02 — four-arm heuristic):
 *   1. HTTP 404 → `already_in_target_state: true`, exit 0.
 *   2. HTTP 400 with body matching `/not found|does not exist/i` →
 *      `already_in_target_state: true`, exit 0.
 *   3. HTTP 400 containing `UserCannotDeleteWorkReport` → hard
 *      `FreeloApiError` (ACL — caller lost rights / never had them).
 *   4. Other non-2xx → re-throw `FreeloApiError`.
 *
 * Mirrors `src/commands/tasks/delete.ts` byte-compat modulo the field rename
 * (`task_id` → `report_id`) and the four-arm catch.
 *
 * Output schema: `freelo.reports.delete/v1`.
 */
...
import { deleteReport, reportPath } from '../../api/reports.js';
...
import { renderReportsDeleteHuman } from '../../ui/human/reports-delete.js';
...
export const meta: CommandMeta = {
  outputSchema: 'freelo.reports.delete/v1',
  destructive: true,
};

const SCHEMA: SchemaString = 'freelo.reports.delete/v1';
...
/* ---------------------------------------------------------------------------
 *  Per-line NDJSON schema. Mirrors `tasks/delete.ts`.
 * ------------------------------------------------------------------------- */
...
function parsePositiveInt(label: string, raw: string): number {
  ...
  if (!Number.isInteger(n) || n < 1) {
    throw new ValidationError(`${label} must be a positive integer.`, {
      hintNext: `${label} is the numeric report id from \`freelo reports list\`.`,
    });
  }
  return n;
}

function collectReportId(raw: string, prev: number[] | undefined): number[] {
...
function parseIdsFlag(raw: string): number[] {
  ...
  if (tokens.length === 0) {
    throw new ValidationError('--ids requires at least one id.', {
      hintNext: '--ids takes a comma- or space-separated list of numeric report ids.',
    });
  }
...
export function registerDelete(
  ...
  const cmd = reports
    .command('delete')
    .description(
      'Delete one or more work reports. Destructive — requires --yes (non-TTY) or interactive confirmation (TTY). 404 (and 400 with "not found" body) treated as idempotent already-deleted; 400 with UserCannotDeleteWorkReport surfaces as a hard ACL error.',
    )
    .argument('[id...]', 'One or more numeric report ids (positional).', collectReportId)
    .option(
      '--ids <list>',
      'Comma- or space-separated list of report ids (mutex with positional <id> and --stdin).',
    )
    .option(
      '--stdin',
  ...
  // NOTE: `--yes` / `-y` is the global root flag; resolved via `resolveYesFlag`.
  attachMeta(cmd, meta);
  ...
  cmd.action(async (ids: number[] | undefined, opts: DeleteOpts, cmdCtx: Command) => {
    ...
    try {
      ...
      if (resolvedIds.length === 0) {
        // No ids resolved → silent success (matches tasks/delete).
        return;
      }
...
function validateInputSources(ids: number[] | undefined, opts: DeleteOpts): void {
  ...
  if (sourceCount === 0) {
    throw new ValidationError('No report ids supplied.', {
...
async function runIdList(
  ...
  for (let i = 0; i < ids.length; i += 1) {
    ...
    try {
      await runOneId(reportId, opts, appConfig, client, /* lineIndex */ undefined);
    } catch (err: unknown) {
      const typed = toBaseError(err);
      writeBatchError(typed, i, reportId, mode, /* fromStdin */ false);
...
async function runBatchFromStdin(
  ...
  for (let i = 0; i < allLines.length; i += 1) {
    ...
    const reportId = result.value.id;
    try {
      const client = await getClient();
      await runOneId(reportId, opts, appConfig, client, /* lineIndex */ i);
    } catch (err: unknown) {
      const typed = toBaseError(err);
      writeBatchError(typed, i, reportId, mode, /* fromStdin */ true);
      exitAcc.observe(typed.exitCode);
    }
  }
...
/* ---------------------------------------------------------------------------
 *  Per-id flow — dry-run / DELETE / four-arm idempotency
 * ------------------------------------------------------------------------- */
...
async function runOneId(
  ...
  if (opts.dryRun === true) {
    ...
    const envelope: Envelope<ReportsDeleteData> = {
      schema: SCHEMA,
      data,
      dry_run: true,
    };
  ...
  // ---- 2. Live DELETE.
  try {
    const result = await deleteReport(client, reportId, {
    ...
    // ---- 3. Four-arm idempotency heuristic (decision 02).
...
/**
 * Decide whether a `FreeloApiError` represents a second-delete on a resource
 * that's already gone (idempotent skip) vs. a real failure. Four-arm matrix
 * per spec 0034 decision 02:
 *
 *   1. HTTP 404 → idempotent skip.
 *   2. HTTP 400 with body text matching `/not found|does not exist/i` →
 *      idempotent skip.
 *   3. HTTP 400 containing `UserCannotDeleteWorkReport` → NOT idempotent;
 *      ACL-shaped error stays observable.
 *   4. Otherwise → NOT idempotent.
 *
...
export function isIdempotentDeleteSkip(err: FreeloApiError): boolean {
  ...
  const haystack = stringifyErrorBody(err);
  // Arm 3 short-circuits arm 2: if the ACL marker is present, NOT idempotent.
  if (/UserCannotDeleteWorkReport/i.test(haystack)) return false;
  if (/not found|does not exist/i.test(haystack)) return true;
  return false;
}

/**
 * Concatenate every text-bearing piece of a `FreeloApiError` into a single
 * string for regex inspection. Covers `errors[]`, `rawBody.errors[]`,
 * `rawBody.message`, and `message` so the heuristic isn't sensitive to which
 * field Freelo populated this time.
 */
function stringifyErrorBody(err: FreeloApiError): string {
  const parts: string[] = [];
...
function confirmMessage(count: number): string {
  if (count === 1) return 'Delete 1 report?';
  return `Delete ${count} reports?`;
}

function writeEnvelope(
  envelope: Envelope<ReportsDeleteData>,
  mode: 'human' | 'json' | 'ndjson',
): void {
  if (mode === 'human') {
    const line = renderReportsDeleteHuman(envelope.data);
