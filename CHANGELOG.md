...
- 33c17c4: Declare `Subtask.type` (`subtask` | `taskcheck`) on `SubtaskSchema`.
...
- 4a55164: Add `freelo task-labels merge --from <uuid>... --to <uuid>` (M06, spec 0068) — consolidate
  ...
  **New envelope schema `freelo.task_labels.merge/v1`** (additive; no existing schema changes).
  ...
  A 404 (label missing _or_ not owned by you — the API collapses the two) stays an error and is never
...
- 30e2def: fix(tasks): `tasks list --project <p> --tasklist <t>` now returns tasks in the tasklist's manual board order
