...
- **Notifications wire shape diverges from the published spec** (verified 2026-04-29 via the official Freelo MCP at `freeloapp/mcp` and reproduced against live API; re-checked against the 2026-08-24 upstream refresh):
  - ~~`GET /all-notifications?only_unread=…` requires the **string `"1"`** (or `"0"`), NOT the canonical boolean `true`/`false` the YAML documents.~~ **RESOLVED upstream as of the 2026-08-24 refresh** — the published YAML now documents `only_unread` as `type: integer, enum: [0, 1]` with an explicit note that `true`/`false` silently fall back to the default. No local override needed anymore; `buildQuery` boolean handling remains correct for every other flag.
  - The mark endpoints are `POST /notification/{id}/mark-read` and `POST /notification/{id}/mark-unread` — **without** the `as-` infix the YAML shows. Calling `/mark-as-read` returns 200 but is a no-op (does not actually flip the read flag). **Still unresolved** as of the 2026-08-24 refresh — upstream still documents the `-as-` infix form.
  - The still-open quirk is flagged inline in `docs/api/freelo-api.yaml` (search for `WIRE QUIRK`). Cross-references: `https://github.com/freeloapp/mcp/blob/main/src/tools/notifications/get-notifications.ts`, `mark-notification-read.ts`, `mark-notification-unread.ts`.
  - Implication for our wire code: the `notifications.ts` path helpers must keep dropping the `-as` infix; the `onlyUnread` boolean-to-string conversion is no longer necessary to treat as a local correction, though the runtime behavior (send `1`/`0`) is unchanged.
...
- **`GET /project-labels/find-available` and task-inline labels are separate buckets.**
  ...
  - CLI implication: `GET /project-labels/find-available` is **not** a reliable name→uuid resolver for **task**-labels. This quirk is about the *project*-labels endpoint and still holds for it.
  - **Resolved for task-labels (M04, spec 0062, shipped).** Use `GET /task-labels/find-available` — a **distinct** endpoint (plural `task-labels`, not `project-labels`), added in the 2026-08-24 spec refresh, surfaced as `freelo task-labels find [--project <id>]`. It returns `{labels:[{uuid,name,color}]}` sorted by name, optionally scoped by `?project_id=`. The old workarounds — (a) scanning `GET /all-tasks`, (b) round-tripping through `POST /task-labels/add-to-task` — are **obsolete**; do not reach for them.
  - Do not conflate the two endpoints. Task labels are **uuid**-keyed and have no `id`; project labels are **id**-keyed. `/task-labels/find-available` accepts `project_id`; `/project-labels/find-available` accepts no query parameters at all.
  - `/task-labels/find-available` returns `{"labels":[]}` (HTTP 200, **not** an error) when `project_id` is inaccessible/nonexistent *and* when the caller has no accessible projects. It does not distinguish the cases — treat empty as a legitimate result, never as a failure.
