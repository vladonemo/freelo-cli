...
- **`GET /project-labels/find-available` and task-inline labels are separate buckets.**
  ...
  - CLI implication: `find-available` is **not** a reliable name→uuid resolver for task-labels — but as of the 2026-08-24 refresh, a **dedicated** resolver now exists: `GET /task-labels/find-available` (a distinct, newly-documented endpoint — plural `task-labels`, not `project-labels`). See `docs/roadmap-migration-2026-08.md` M04. The two round-trip workarounds below are no longer the only option once that slice ships; they remain accurate for `GET /project-labels/find-available` (project-level labels) which this note is actually about.
  - Until M04 ships: to resolve a task-label name to its uuid, either (a) scan tasks via `GET /all-tasks` (expensive) or (b) round-trip via `POST /task-labels/add-to-task` (which accepts name-mode) and read the resolved uuid from the next `GET /task/{id}`.
