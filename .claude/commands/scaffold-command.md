...
2. Create files using the project's established patterns (read an existing neighboring command file as the reference):
   - `src/commands/<name>.ts` — registers the command with a `register(program)` export, wires to `src/api/<name>.ts`, supports `--output`
   - `src/api/<name>.ts` — typed endpoint function(s) with zod schemas in `src/api/schemas/<name>.ts`
   - `test/commands/<name>.test.ts` — integration test skeleton with MSW handlers for 200 and one error case
