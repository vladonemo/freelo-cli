...
**Outcome:** See one project's full metadata and workers.
**Endpoints:** `GET /project/{id}`, `GET /project/{id}/workers`.
**CLI:** `freelo projects show <id> [--with workers]`
**Depends on:** R03.
**Note:** Per-project labels read is deferred — no documented Freelo endpoint exposes the labels assigned to a project (see spec 0013 §6). Tracked as future R04.5.
