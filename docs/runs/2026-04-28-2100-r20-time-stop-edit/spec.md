...
freelo time edit [--task <id>] [--no-task] [--note <str>] [--dry-run]
```
...
```
$ freelo time edit
# stderr: VALIDATION_ERROR — `time edit` requires at least one of --task / --no-task / --note.
# exit 2
```

Mutex pair `--task` / `--no-task` is a Commander negatable: passing both is a usage error (exit 2). Decision 4 + decision 3.
...
$ freelo time edit --no-task --output json
...
# Conflict (--task + --no-task):
$ freelo time edit --task 4567 --no-task
# exit 2 — VALIDATION_ERROR
...
export type EditTimerInput = {
  /** undefined = don't touch; number = assign; null = disassociate (--no-task). */
  taskId?: number | null;
  note?: string;
};
...
Same shape, plus mutex validation between `--task` and `--no-task`, plus the empty-edit rejection:

```ts
.command('edit')
  .description('Edit the active time tracking session in flight (--task / --no-task / --note). Returns 409 with a friendly hint when no session is running. At least one of --task / --no-task / --note is required.')
  .option('--task <id>', '...', parseTaskFlag)        // mutex with --no-task
  .option('--no-task', 'Disassociate the session from any task (sends task_id: null).')
  .option('--note <str>', '...')
  .option('--dry-run', '...')
  .action(async (opts: { task?: number; noTask?: boolean; note?: string; dryRun?: boolean }) => {
    // Validate: --task + --no-task is a usage error (decision 3).
...
| `time edit` with no flags                                        | `ValidationError` "requires at least one of --task / --no-task / --note", exit 2 (decision 4)                       |
| `time edit --task 4567 --no-task`                                | `ValidationError` "--task and --no-task are mutually exclusive", exit 2 (decision 3)                                |
...
| `time edit --no-task` (only)                                     | Wire body: `{ task_id: null }`. envelope `applied_changes.task_id = null`, no `note` key. exit 0                     |
...
**Decision:** Defer to R20.5. Ship R20 with the documented edit body (`--task` / `--no-task` / `--note`).
...
### Decision 3 — `--task` / `--no-task` mutex pair on `time edit`

**Question:** OpenAPI documents `task_id` as nullable on edit. How does the CLI surface "set to null"?

**Decision:** Add two flags: `--task <id>` (assign) and `--no-task` (disassociate, sends `task_id: null`). They are mutually exclusive — both supplied → `ValidationError` exit 2.
...
- Reuse Commander's `--no-<flag>` pattern alone (no `--task <id>`) — rejected; two flags collapse the assignment direction onto one knob, which would surprise users.

**Rationale:** Commander supports negatable flags natively (`--no-task` becomes `noTask: true`). Mutex is a one-line check. Same pattern used in `tasks edit --due` / `--no-due` (R10).
...
**Decision:** Usage error. `ValidationError` exit 2 with hint `"requires at least one of --task / --no-task / --note"`.
...
**Decision:** Only the keys the user passed. Mirrors the wire body shape: if `--task` and `--no-task` are both omitted, the wire body has no `task_id` key, and the envelope's `applied_changes` has no `task_id` key.
...
- Always echo both, with the wire-body shape (`task_id` absent if not touched, present-and-null if `--no-task`, present-and-int if `--task`) — same as the chosen path; just rephrased.
...
| `src/commands/time/edit.ts`                       | Leaf — `--task` / `--no-task` / `--note` / `--dry-run`. Mutex validation, empty-edit rejection, 409 hint rewrite.               |
...
| `test/commands/time/edit.test.ts`                 | Integration: each flag combo, mutex `--task`/`--no-task` (exit 2), empty-edit (exit 2), `--dry-run`, 409 hint, 401, 5xx.        |
...
- `buildEditTimerBody` — empty input → `{}`; `--task` only → `{ task_id }`; `--no-task` (taskId: null) → `{ task_id: null }`; `--note` only → `{ note }`; both → both keys.
...
- New `--task` / `--no-task` mutex pair on `time edit` (rationale: capability documented in OpenAPI but missing from roadmap text).
